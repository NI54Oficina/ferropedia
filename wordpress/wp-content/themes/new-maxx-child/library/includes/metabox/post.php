...
function kopa_edit_extra_post_fields() {
    ...
    // add_meta_box(
    //     'kopa-post-thumbnail-custom', // this is HTML id of the box on edit screen
    //     'Custom Thumbnail', // title of the box
    //     'kopa_custom_thumbnail', // function to be called to display the checkboxes, see the function below
    //     'post', // on which edit screen the box should appear
    //     'normal', // part of page where the box should appear
    //     'default'      // priority of the box
    // );
...
/*
//display the metabox custom image
function kopa_custom_thumbnail($kopa_post) {
    $thumbnail_sizes = kopa_get_size_for_custom_thumbnail();
    ?>
    <div id="kopa-post-edit" class="kopa-post-edit kopa-content-main-box">
        <?php
            foreach($thumbnail_sizes as $k => $v){
                $post_name = 'thumbnail_'.$v['size'];
                $thumbnail = get_post_meta( get_the_ID(), $post_name, true );
            ?>
                <div class="kopa-opt-fields">
                    <div class="kopa-opt-left">
                        <?php echo $v['des']; ?><br/>
                        <span class="kopa-field-size">Size: <?php echo $v['size']; ?> (px)</span>
                    </div>
                    <div class="kopa-opt-right">
                        <input class="left" type="text" value="<?php echo $thumbnail; ?>" id="<?php echo $post_name;?>" name="<?php echo $post_name;?>">
                        <button class="btn-success upload_image_button" alt="<?php echo $post_name;?>"><i class="icon-circle-arrow-up"></i><?php _e('Upload', 'newsmaxx'); ?></button>
                    </div>
                </div>
                                    ...
                                    <div class="kopa-sidebar-box kopa-element-box">
                                        ...
                                        foreach ($kopa_sidebar as $sidebar_list_key => $sidebar_list_value) {
                                            ...
                                            if ($layout_key === $kopa_post_setting['layout_id']) {
                                                if (isset($kopa_post_setting['sidebars'][$postion_key]) && $sidebar_list_key === $kopa_post_setting['sidebars'][$postion_key]) {

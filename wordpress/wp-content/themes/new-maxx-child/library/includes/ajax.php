        ...
        if (!empty($_POST['removed_sidebar_id'])) {
            ...
            if ($removed_sidebar_id === '0') {
                ...
                if ($found_sidebar) {
                    ...
                    $found_setting = false;
                    $found_is_used = is_active_sidebar('sidebar_'.$removed_sidebar_id);;
                    ...
                    } elseif($found_is_used){
                        echo json_encode(array("is_exist" => true, "error_message" => "You can not remove this sidebar. It has some widgets"));
                    }
                    else {
...
if ( !function_exists('kopa_update_sidebar')) {

    function kopa_update_sidebar() {
        check_ajax_referer('kopa_rename_sidebar', 'ajax_nonce');
        if (!empty($_POST['removed_sidebar_name']) && !empty($_POST['removed_sidebar_des'])) {
            $kopa_sidebar_name = ($_POST['removed_sidebar_name']);
            $kopa_sidebar_des = ($_POST['removed_sidebar_des']);
            $remove_sidebar_id = 'sidebar_'.($_POST['removed_sidebar_id']);
            $kopa_sidebar = get_option("kopa_sidebar", array());
            $kopa_sidebar_new = array();
            foreach( $kopa_sidebar as $k => $v ){
                if ( $remove_sidebar_id == $v['sidebar_key']) {
                     $kopa_sidebar_new[] =  array(
                         'sidebar_key' => $remove_sidebar_id,
                         'sidebar_name' => $kopa_sidebar_name,
                         'sidebar_des' => $kopa_sidebar_des
                     );
                }else{
                    $kopa_sidebar_new[] = $v;
                }
            }
            update_option("kopa_sidebar", $kopa_sidebar_new);
            echo json_encode(array("is_exist" => false, "error_message" => "successfull", "kopa_sidebar"=>$kopa_sidebar_new));
        }
        exit();
    }

    add_action('wp_ajax_kopa_update_sidebar', 'kopa_update_sidebar');
}
////////////////////////////////////////////////////////
if (!function_exists('kopa_add_sidebar')) {
    ...
    function kopa_add_sidebar() {
        if (!wp_verify_nonce($_POST['wpnonce'], 'save_sidebar_setting'))
            exit();
        if (!empty($_POST['new_sidebar_name']) && !empty($_POST['new_sidebar_des'])) {
            $kopa_sidebar_name = ($_POST['new_sidebar_name']);
            $kopa_sidebar_des = ($_POST['new_sidebar_des']);
            $kopa_sidebar = get_option("kopa_sidebar", array());
            //$sidebar_id = strtolower(trim(str_replace(" ", "_", $kopa_sidebar_name)));
            $sidebar_last_index = count($kopa_sidebar);
            $sidebar_fount_index = '';
            $found_sidebar = false;
            foreach ($kopa_sidebar as $e_sidebar_id => $e_sidebar_value) {
                if ($kopa_sidebar_name === $e_sidebar_value['sidebar_name']) {
                    $found_sidebar = true;
                    $sidebar_fount_index = $e_sidebar_value['sidebar_key'];
                }
            }
            if ($found_sidebar) {
                $error_message = 'The sidebar name "' . $kopa_sidebar_name . '" already exist!';
                echo json_encode(array("is_exist" => true, "error_message" => $error_message, "sidebar_id" => $sidebar_fount_index));
            } else {
                echo json_encode(array("is_exist" => false, "error_message" => "", "sidebar_id" => $sidebar_last_index));
                $kopa_sidebar[] = array(
                    'sidebar_key' => "sidebar_".$sidebar_last_index,
                    'sidebar_name' => $kopa_sidebar_name,
                    'sidebar_des' => $kopa_sidebar_des
                );
                update_option("kopa_sidebar", $kopa_sidebar);
...
if (!function_exists('save_layout')) {
    ...
    function save_layout() {
        ...
        $before_setting = get_option('kopa_setting');
        ...
        if (!empty($_POST)) {
            ...
            $after_setting = array();
            $after_setting['layout_id'] =  $layout_id;

            $kopa_layout = unserialize(KOPA_LAYOUT);
            $kopa_position = $kopa_layout[$layout_id]['positions'];

            $new_sidebars = $_POST['kopa_sidebar'];
            $kopa_sidebar = array();
            ...
            if ( !empty($before_setting['sidebars'])) {
                if (in_array($layout_id, $before_setting['sidebars'])){
                    $after_setting['sidebars'][$layout_id] =  $kopa_sidebar;
                } else {
                    $after_setting['sidebars'][$layout_id] =  $kopa_sidebar;
                }
            } else {
            ...
            $kopa_setting[$template_id]  =  $after_setting;
...
function kopa_layout_page($_kopa_template_id) {
    ...
    $current_layout_sidebar = $kopa_setting[$_kopa_template_id]['sidebars'][$kopa_setting[$_kopa_template_id]['layout_id']];
            ...
            <div class="kopa-page-header clearfix">
                ...
                <div class="pull-right">
                    <div class="kopa-copyrights">
                        <?php $kopa_url = 'http://kopatheme.com';?>
                                        ...
                                        <div class="<?php echo 'kopa-sidebar-box-wrapper sidebar-position-' . $layout_key; ?>">
                                            ...
                                            foreach ($layout_value['positions'] as $postion_key => $postion_id) {
                                                ...
                                                <div class="kopa-sidebar-box kopa-element-box">
                                                    ...
                                                    <?php //var_dump($kopa_setting[$_kopa_template_id]['sidebars'][$layout_key][$postion_key]); ?>
                                                    <?php
                                                    echo '<select class="kopa-sidebar-select" autocomplete="off">';
                                                    foreach ($kopa_sidebar as $sidebar_list_key => $sidebar_list_value) {
                                                        $__selected_sidebar = '';
                                                            /*if ($kopa_setting[$_kopa_template_id]['sidebars'][$layout_key][$postion_key] == $sidebar_list_value['sidebar_key'] ) {

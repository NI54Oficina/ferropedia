...
function kopa_add_sidebar_clicked(obj) {
    if (jQuery("#kopa-sidebar-new").val().length > 0 && jQuery("#kopa-sidebar-des-new").val().length > 0) {
        var new_sidebar_name = jQuery("#kopa-sidebar-new").val();
        var new_sidebar_des = jQuery("#kopa-sidebar-des-new").val();
        ...
        jQuery.ajax({
            ...
            data: {
                ...
                new_sidebar_des: new_sidebar_des,
            ...
            success: function(data) {
                ...
                if (data.is_exist) {
                    ...
                    var temp_html = '<tr>' +
                        ...
                        '</td>' +
                        '<td>' +
                        '<a href="#TB_inline?width=600&height=350&inlineId=my-edit-content" onclick="kopa_show_edit_form(jQuery(this), \''+new_sidebar_name +'\', \''+new_sidebar_des +'\', \'' + data.sidebar_id +'\')" class="thickbox button button-basic button-icon"><i class="icon-pencil"></i></a>' +
                        '</td>' +
                        '</tr>';
                    jQuery("#kopa-sidebar-list").append(temp_html);
                    jQuery("#kopa-sidebar-new").val('');
                    jQuery("#kopa-sidebar-des-new").val('');
                }
            },
        ...
        alert("You have not enter sidebar name or sidebar description!");
    }
    return false;
}

function kopa_show_edit_form(obj, sidebar_name, sidebar_des, removed_sidebar_id ) {
    jQuery('#kopa-sidebar-name').val(sidebar_name);
    jQuery('#kopa-sidebar-des').val(sidebar_des);
    jQuery('#kopa-remove-sidebar-key').val(removed_sidebar_id);
}

function kopa_edit_sidebar_clicked(obj){
    if (jQuery("#kopa-sidebar-name").val().length > 0 && jQuery("#kopa-sidebar-des").val().length > 0) {
        var kopa_sidebar_id = jQuery('#kopa-remove-sidebar-key').val();
        var kopa_sidebar_name = jQuery('#kopa-sidebar-name').val();
        var kopa_sidebar_des = jQuery('#kopa-sidebar-des').val();
        var answer = confirm("Are you sure to update this sidebar?");
        if (answer === true) {
            tb_remove();
            jQuery.ajax({
                type: 'POST',
                ...
                dataType: 'json',
                data: {
                    action: "kopa_update_sidebar",
                    removed_sidebar_id: kopa_sidebar_id,
                    removed_sidebar_name: kopa_sidebar_name,
                    removed_sidebar_des: kopa_sidebar_des,
                ...
                success: function(data) {
                    ...
                    if (data.is_exist) {
                        ...
                        var temp_html = '';
                        for(var index=0; index < data.kopa_sidebar.length; index++ ){
                            var obj = data.kopa_sidebar[index];
                            if ( 'sidebar_0' == obj['sidebar_key']) {
                                continue;
                            }
                            var sidebar_key = obj['sidebar_key'].split("_");
                            temp_html += '<tr>' +
                                '<td>' + obj['sidebar_name'] +
                                '</td>' +
                                '<td>' + obj['sidebar_des'] +
                                '</td>' +
                                '<td>' +
                                '<a onclick="kopa_remove_sidebar_clicked(jQuery(this),\''+ sidebar_key[1] +'\')" title="" lang="" rel="tooltip" class="button button-basic button-icon" data-original-title="Remove"><i class="icon-trash"></i></a>' +
                                '</td>' +
                                '<td>' +
                                    '<a href="#TB_inline?width=600&height=350&inlineId=my-edit-content" onclick="kopa_show_edit_form(jQuery(this), \''+obj['sidebar_name'] +'\', \''+obj['sidebar_des'] +'\', \''+sidebar_key[1] +'\')" class="thickbox button button-basic button-icon"><i class="icon-pencil"></i></a>' +
                                '</td>' +
                                '</tr>';
                        }

                       jQuery('#kopa-sidebar-list').html(temp_html);
                    }
                },
                complete: function(data) {
                    jQuery('#kopa-loading-gif').hide();
...
function show_on_check_social_link(obj){
    var $this = jQuery(obj);
    var dataWrapper = $this.data('wrapper');

    if ($this.prop('checked')) {
        jQuery('#'+dataWrapper+' *').attr('disabled', true);

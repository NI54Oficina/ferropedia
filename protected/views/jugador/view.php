...
<h1>Jugador</h1> <?php if(is_user_logged_in()){ ?><a href="<?php echo Yii::app()->request->baseUrl; ?>/jugador/update/<?php echo $model->id; ?>">Editar</a> / <a href="<?php echo Yii::app()->request->baseUrl; ?>/jugador-<?php echo $model->id; ?>" target="_blank">Ver en Ferropedia</a><br><br><?php } ?>
...
<?php } ?>
<p><strong>Debut:</strong> <?php $debut= DataExtra::model()->findByAttributes(array("model"=>"Jugador","modelId"=>$model->id,"titulo"=>"Debut"));
if(isset($debut)){echo $debut->texto; } ?></p>
<p><strong>Último Partido:</strong> <?php 
		$ultimo= DataExtra::model()->findByAttributes(array("model"=>"Jugador","modelId"=>$model->id,"titulo"=>"Último partido"));
...
<ul>
<?php foreach($model->data as $data){ if($data->titulo=="Torneo"||$data->titulo=="Debut"||$data->titulo=="Último partido"){continue;}?>
	<li>
...
foreach($model->logros as $logro){
	...
	<div style="display:inline-block;width:100px;">
		...
		<a href="<?php echo home_url(); ?>/jugador/deleteLogro/<?php echo $logro->id; ?>">Borrar</a>
	</div>
	<?php
}
?>
...
<br>
<hr>
<form id="logro"name="logro" action="<?php echo Yii::app()->request->baseUrl; ?>/jugador/crearLogro" method="post" >
	<input name="model" value="<?php echo get_class($model); ?>" type="hidden" />
  <input type="hidden" name="modelId"  value="<?php echo $model->id; ?>" />
...
<hr>
<br>
<a class="button-like" href="<?php echo Yii::app()->request->baseUrl; ?>/jugador/torneos/<?php echo $model->id; ?>">Ver Campañas</a>
<br>
<br>
